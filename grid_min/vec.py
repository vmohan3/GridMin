...
def add(u,v):
	...
	assert u.D == v.D
	return(Vec(u.D,dict((i,getitem(u,i)+getitem(v,i)) for i in u.D)))
...
def dot(u,v):
	...
	sum = 0
	for i in u.D:
		sum+=getitem(u,i)*getitem(v,i)
	return sum

def scalar_mul(v, alpha):
    "Returns the scalar-vector product alpha times v"
    return(Vec(v.D,dict((i,alpha*getitem(v,i)) for i in v.D)))

def neg(v):
    "Returns the negation of a vector"
    return(Vec(v.D,dict((i,-getitem(v,i)) for i in v.D)))
